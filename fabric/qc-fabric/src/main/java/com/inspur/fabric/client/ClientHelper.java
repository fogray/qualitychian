...
import com.inspur.fabric.channelpeer.IChannelPeerService;
...
public class ClientHelper {
    ...
    private static IChannelPeerService channelPeerService = (IChannelPeerService)ComponentFactory.getBean("channelPeerService");
    ...
    private Channel getQueryChannel(BaseOrg org, HFClient client, String channelName) throws Exception {
        ...
        List<Map<String, Object>> channelPeers = channelPeerService.getChannelOrg(param);
        for (String peerName : org.getPeerNames()) {
            String peerLocation = org.getPeerLocation(peerName);
            boolean has = false;
            for(Map<String,Object> cp : channelPeers) {
            	String pn = (String) cp.get("PEER_NAME");
            	if (peerName.equals(pn)) {
            		has = true;
            		break;
            	}
    ...
    private Channel getChannel(BaseOrg org, HFClient client, String channelName)
        ...
        for (BaseOrg porg : orgs){
            for (String peerName : porg.getPeerNames()) {
                ...
                for(Map<String,Object> cp : channelPeers) {
                	String pn = (String) cp.get("PEER_NAME");
                	System.out.println("----peerName="+peerName+", pn="+pn);
                	if (peerName.equals(pn)) {
                		has = true;
                		break;
                	}
