...
func setTraceInfo1( stub shim.ChaincodeStubInterface, key string, value string, channel string ) (string, error) {
	...
	param1 := toChaincodeArgs("invoke","queryKeys",fmt.Sprintf("{\"selector\":{\"$and\": [{\"START\": {\"$lte\": \"%s\"}}, {\"END\": {\"$gte\": \"%s\"}}]}}",start,end))
	...
	cs := &CodeInfo1{}
	err = json.Unmarshal([]byte(keys),cs)
	if err != nil {
		return "failed", fmt.Errorf("failed! 获取秘钥失败")
	}
	secretKey := cs.SecretKey
	arg2 := value
	res := arg2[0:len(arg2)-1]+`,"SECRETKEY":"`+secretKey+`"}`
	return res, nil
}
...
func main() {
	if err := shim.Start(new(TestChaincode)); err != nil {
		fmt.Printf("Error starting TestChaincode: %s", err)
	}
}