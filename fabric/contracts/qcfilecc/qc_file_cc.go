...
func (t *FileChaincode)Init(stub shim.ChaincodeStubInterface) peer.Response{
	return shim.Success(nil)
}

func (t *FileChaincode)Invoke(stub shim.ChaincodeStubInterface) peer.Response {
    fn, args := stub.GetFunctionAndParameters()
    if fn != "invoke" {
        return shim.Error(fmt.Sprintf("FileChaincode Unknown function call--fn-->%s--args[0]-->%s",fn,args[0]))
    }
    var result string
    var err error
    if args[0] == "setData" {
...
func setData(stub shim.ChaincodeStubInterface, args []string) (string, error) {
	...
	if err != nil {
        return fmt.Sprintf("{\"success\": \"false\",\"msg\":\"FileChaincode setData failed key:%s--value:%s\"}", args[1], args[2]) , nil
    }
...
func getData(stub shim.ChaincodeStubInterface, args []string)(string, error) {
	...
	if err != nil {
        return fmt.Sprintf("{\"success\":\"false\",\"msg\":\"FileChaincode getData failed key:%s\"}", args[1]), nil
    }
    if value == nil {
...
func deleteData(stub shim.ChaincodeStubInterface, args []string)(string, error) {
	...
	if err != nil {
        return fmt.Sprintf("{\"success\":\"false\",\"msg\":\"FileChaincode deleteData failed key:%s\"}", args[1]), nil
    }
...
func main() {
	if err := shim.Start(new(FileChaincode)); err != nil {
		fmt.Printf("FileChaincode Error starting FileChaincode: %s", err)
	}
}